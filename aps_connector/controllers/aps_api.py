...
class ApsApiController(http.Controller):
    ...
    @http.route('/aps/api/v1/health', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    ...
    @http.route('/aps/api/v1/calendar_exceptions', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    ...
    @http.route('/aps/api/v1/maintenance_requests', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    ...
    @http.route('/aps/api/v1/calendars', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    ...
    @http.route('/aps/api/v1/resources', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    def get_resources(self, **kwargs):
        ...
        try:
            ...
            cr = request.env.cr
            # Odoo 19: default_capacity moved to mrp.workcenter.capacity
            # (One2many). The generic default has product_id IS NULL.
            cr.execute("""
                SELECT wc.id, wc.name, wc.resource_calendar_id,
                       COALESCE(wcc.capacity, 1) AS default_capacity,
                       wc.time_efficiency,
                       wc.costs_hour, wc.time_start, wc.time_stop
                FROM mrp_workcenter wc
                LEFT JOIN mrp_workcenter_capacity wcc
                    ON wcc.workcenter_id = wc.id AND wcc.product_id IS NULL
    ...
    @http.route('/aps/api/v1/products', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    ...
    @http.route('/aps/api/v1/boms', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    ...
    @http.route('/aps/api/v1/orders', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    def get_orders(self, **kwargs):
        ...
        SO resolution uses stock.reference relay tables (Odoo 19+).
        """
        try:
            api_key = kwargs.get('api_key')
    ...
    @http.route('/aps/api/v1/operations', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    def get_operations(self, **kwargs):
        ...
        try:
            ...
            aps_to_odoo_state = {
                'PENDING': ['pending', 'waiting', 'blocked'],
                'SCHEDULED': ['ready'],
    ...
    @http.route('/aps/api/v1/inventory', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    ...
    @http.route('/aps/api/v1/purchase_orders', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
    ...
    @http.route('/aps/api/v1/schedule/validate', type='jsonrpc', auth='none', methods=['POST'], csrf=False)
