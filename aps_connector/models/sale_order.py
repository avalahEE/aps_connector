...
def sale_orders_by_mo(cr, mo_ids):
    ...
    The move that created an MO, or the finished move's destination, carries the
    procurement group and through it the sale order; a child MO reaches the order
    through the move that created it, and an MO with no group at all through its origin
    chain. Used by the orders feed and by the write-back, which posts the order's planned
    completion. Returns {mo_id: {'id', 'name', 'customer'}}.
    """
    if not mo_ids:
        return {}
    cr.execute("""
        ...
        origin_so AS (
            SELECT DISTINCT ON (oc.original_id)
                ...
                COALESCE(so_pg.id, so_origin.id) AS sale_order_id,
